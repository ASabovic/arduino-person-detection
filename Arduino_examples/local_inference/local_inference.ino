...
appropriate LED (green if person is detected on the image and red if it is not).
...
The energy-aware task scheduler is able to select tasks based on their priorities, dependencies and constraints. These tasks will be executed only if their required 
voltage thresholds (if there is enough energy for their execution) are reached. Otherwise, the device will go into sleep state, wait the predefined period of time, and check again.
