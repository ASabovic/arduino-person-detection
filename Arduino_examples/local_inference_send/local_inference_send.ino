...
/*
This script enables the person detection model deployment on tiny Arduino Nano 33 BLE board. Once data is captured and processed, it can be used as a input to our tiny ML model
running on the Arduino board. In this case, we show how the inference task can be executed locally on the device. The final result is confirmed by briefly turning on the 
appropriate LED (green if person is detected on the image and red if it is not) and sending it to the IoT gateway (or Cloud) via BLE to be displayed.
...
int V_0; // this is the initial voltage value (measured voltage, equal to V0 in the equation for time)
int V_req; // required voltage threshold (equal to Vt in the equation for time)
...
//      V_0 = read_voltage();
      ...
      else
      {
        while(V_0 < V_req)
        {
          delay(3000); //here will be t added instead of 5 seconds
          V_0 = read_voltage();
        }
        execute(&TSK_LIST[loc]);
        addTask(loc);
...
float time_function(float Ih, float Req, float C, float V1, float V2)
{
...
{
    digitalWrite(4, HIGH);
    int sensorValue = analogRead(A0);//pin 4 on the left side on the breadboard
    int vol = (sensorValue * 3600 / 1024) * 2.6; // equation for integer measured voltage
//    float vol = (sensorValue * 3.6 / 1024) * 2.6;
    digitalWrite(4, LOW);
    return vol;
}
